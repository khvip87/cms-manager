...
- 2026-05-17 — CMS-14 merged via PR [#4](https://github.com/khvip87/cms-database/pull/4). Seed runner shipped with `_cms_seeds (name TEXT PK, ran_at TIMESTAMPTZ)` tracking table, idempotent re-runs, dev/shared split gated by `NODE_ENV` + `CMS_ALLOW_DEV_SEED`. `seeds/shared/001_roles.ts` upserts the 4 baseline `RoleName` values (admin/editor/author/viewer). 4 new integration tests pass against live Docker Postgres (total 10/10). Side fix: dynamic `await import('@prisma/client')` didn't survive Jest's CJS mode without `--experimental-vm-modules`, so switched to a tiny `loadPrismaModule()` helper using `require`. Added `postinstall: prisma generate` so fresh installs are zero-step. Split tsconfig into `tsconfig.json` (broad include for ESLint + typecheck) and `tsconfig.build.json` (rootDir=src for clean dist/ output). `cms-backend/prisma/seed.ts` removal deferred to CMS-16.
- 2026-05-17 — CMS-13 merged via PR [#3](https://github.com/khvip87/cms-database/pull/3). Shipped the full homegrown runner CLI: `cms-db migrate | status | diff | new | adopt | seed`. All 8 ACs verified; 6/6 integration tests green against live Postgres using isolated per-test schemas. Two real bugs caught and fixed during verification: (a) CRLF-vs-LF checksum drift on Windows — fixed by LF-normalising input to `computeChecksum` and adding `.gitattributes` to pin working-tree line endings; (b) `prisma migrate diff` always emitted `DROP TABLE "_cms_migrations"` because the runner's tracking table isn't in `schema.prisma` — `diff.ts` now filters that statement out before writing migrations, otherwise every generated migration would have dropped the runner's own tracking table on apply.
